#! /usr/bin/env python3
sys.path.insert(1, "../decision_tree")
import argparse
import hashlib
import json
import os
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Tune PeleLMeX SUNDIALS configuration with GPTune"
    )

    parser.add_argument(
        "tuning_spec", type=str, help="Path to tuning problem sepcification file"
    )
    parser.add_argument(
        "-ns",
        "--samples-per-task",
    ...
    parser.add_argument(
        "-ttols", "--tune-tolerances", action="store_true", dest="tune_tolerances"
    )
...
def get_parmparse_options(point):
    params = {
        get_parmparse_key(k): get_parmparse_value(v)
        for k, v in point.items()
        if k != "metadata" and k != "pelelmex-inputs"
    }
...
def main():
    ...
    decision_tree_path = decision_tree.expand_path(
        meta_config_dict["decision_tree_path"]
    )
    tuning_params_list = path_params(decision_tree.graph(), decision_tree_path)
    if args.tune_tolerances:
        tuning_params_list.extend(
            [
                Real(1e-4, 1e-12, transform="normalize", name="atol"),
                Real(1e-2, 1e-10, transform="normalize", name="rtol"),
            ]
        )
    ...
    input_space = Space(
        [
            Categoricalnorm(
                meta_config_dict["inputs"], transform="onehot", name="pelelmex-inputs"
            )
        ]
    )
    ...
    data, modeler, stats = gt.MLA(
        NS=NS, NI=NI, Igiven=giventask, NS1=NS1, T_sampleflag=[True] * NI
    )
    print("stats: ", stats)
    ...
    for tid in range(NI):
        ...
        print(
            "    Popt ",
            data.P[tid][np.argmin(data.O[tid])],
            "Oopt ",
            min(data.O[tid])[0],
            "nth ",
        ...
        if args.sensitivity_analysis:
            ...
            sensitivity_data = SensitivityAnalysis(
                problem_space=problem_space,
                input_task=input_tasks,
