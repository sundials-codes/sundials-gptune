...
def parse_args():
	...
	parser.add_argument('-optimization', type=str,default='GPTuneBand', help='Optimization algorithm (opentuner, hpbandster, GPTune)')
...
def get_tols_from_budget(budget):
	if budget == 1:
		return [1e-8,1e-3]
	elif budget == 3:
		return [1e-9,1e-4]
...
def execute(params):
	...
	print('budget:')
	print(params['budget'])
	tols = get_tols_from_budget(params['budget'])
	atol = tols[0]
	rtol = tols[1]
	
	# Build up command with command-line options from current set of parameters
	argslist = ['mpirun', '-n', str(nodes*cores), diffusion2Dfullpath, '--nx', '150', '--ny', '150',
		'--atol', str(atol),
		'--rtol', str(rtol),
...
def main():
	...
	# Smallest factor for fidelity budget
	options['budget_min'] = 1
	# Largest factor for fidelity budget
	options['budget_max'] = 9
	# Multiplicative factor determining budget levels
	...
	if(TUNER_NAME=='GPTuneBand'):
		...
		(data, modeler, stats) = gt.MB_LCM(NS=NS, Igiven=giventask, NS1=int(NS/2), Pdefault=None)
