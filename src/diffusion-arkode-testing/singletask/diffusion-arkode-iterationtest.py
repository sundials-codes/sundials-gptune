...
def execute(params):
	...
	argslist = ['mpirun', '-n', str(nodes*cores), diffusion2Dfullpath, '--nx', '128', '--ny', '128',
		...
		#'--atol', str(10**-params["atolexp"]),
		#'--rtol', str(10**-params["rtolexp"]),
...
def main():
	...
	parameter_space = Space([
		...
		#Real(1, 10, transform="normalize", name="atolexp"),
		#Real(1, 5, transform="normalize", name="rtolexp"),
	...
	if(TUNER_NAME=='GPTune'):
		...
		for num_points in [30, 40, 50, 60, 70, 80, 90]: 
			NS = num_points
			...
			print('    Popt ', data.P[0][np.argmin(data.O[0])], 'Oopt ', min(data.O[0])[0], 'nth ', np.argmin(data.O[0]))
