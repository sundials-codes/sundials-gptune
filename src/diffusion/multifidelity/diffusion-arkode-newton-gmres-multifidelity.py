...
def main():
    ...
    input_space = Space([Categoricalnorm(["diffusion-arkode-newton-gmres-multifidelity"], transform="onehot", name="problemname")])
    ...
    giventask = [['diffusion-arkode-newton-gmres-multifidelity']]
    NI=len(giventask) 
    NS=nrun
    ...
    if(TUNER_NAME=='GPTune'):
        ...
        for tid in range(NI):
            ...
            if plot_runtime:
                ...
                plt.savefig('diffusion-arkode-newton-gmres-runtime.png')
                plt.close()
            ...
            if plot_params:
                plot_datas = [
                    ...
                    { 'name': 'maxl', 'values': [ elem[5] for elem in data.P[tid] ] },
                    { 'name': 'epslin', 'values': [ elem[6] for elem in data.P[tid] ] }
                ]
                for plot_data in plot_datas:
                    ...
                    plt.savefig('diffusion-arkode-newton-gmres-' + plot_data['name'] + '.png')
                    plt.close()
