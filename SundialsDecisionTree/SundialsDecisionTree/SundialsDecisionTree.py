...
class DecisionTree:
    ...
    def __init__(self):
        self.choose_implicit_or_explicit = DecisionTreeNode(
            label="implicit or explicit?",
            tuning_params=[
                # gpt.Categoricalnorm(
                #     ["implicit", "explicit"],
                #     transform="onehot",
                #     name="implicit or explicit?",
                # )
            ],
        )

        self.implicit = DecisionTreeNode(
            label="implicit methods",
            tuning_params=[
        ...
        self.implicit_bdf = DecisionTreeNode(
            label="BDF",
            tuning_params=[
        ...
        self.implicit_adams = DecisionTreeNode(
            label="Adams",
            tuning_params=[
        ...
        self.choose_nonlinear_solver = DecisionTreeNode(
        ...
        self.fixed_point = DecisionTreeNode(
            label="fixed_point",
            tuning_params=[
        ...
        self.choose_matrix_based_or_free = NoParamNode(
            label="matrix-based or matrix-free?"
        )

        self.matrix_based = DecisionTreeNode(
            label="matrix_based",
            tuning_params=[
        ...
        self.matrix_based_direct = DecisionTreeNode(
            label="direct",
            tuning_params=[
                gpt.Categoricalnorm(
                    ["magma_batched_lu"], transform="onehot", name="linear_solver"
                ),
            ],
        )

        self.matrix_based_iterative = DecisionTreeNode(
            label="iterative",
            tuning_params=[
                gpt.Categoricalnorm(
                    ["ginkgo_GMRES", "ginkgo_BICGSTAB"],
                    transform="onehot",
                    name="linear_solver",
                )
            ],
        ...
        self.preconditioner = DecisionTreeNode(
            label="preconditioner",
            tuning_params=[
                # TODO(CJB): add preconditioner params -
                # will need to think about how this works with general SUNDIALS
            ],
        )

        self.matrix_free = DecisionTreeNode(
            label="matrix_free",
            tuning_params=[
                gpt.Categoricalnorm(
                    ["GMRES", "BCGS"], transform="onehot", name="linear_solver"
                ),
                # TODO(CJB): need a sensible way to modify ranges for parameters from
                # tuning driver since an appropriate range is not the same for all
                # problems
                # gpt.Integer(3, 500, transform="normalize", name="maxl")
                gpt.Integer(
                    1, 500, transform="normalize", name="maxl"
                ),  # special for PELE
            ],
        )

        self.explicit = DecisionTreeNode(
            label="explicit",
            tuning_params=[
                gpt.Categoricalnorm(
                    [
        ...
        self.dt_graph.add_edge(
            self.choose_direct_or_iterative, self.matrix_based_iterative
        )
        self.dt_graph.add_edge(
            self.choose_direct_or_iterative, self.matrix_based_direct
        )
    ...
    def expand_path(self, list_path):
        ...
        for ind, nd in enumerate(list_path):
            if nd == "*":
                for nd in nx.bfs_successors(
                    self.dt_graph, getattr(self, list_path[ind - 1])
                ):
    ...
    def draw(self, save_fig=True, save_path="decision_tree.png"):
        ...
        #     G, pos, labels={edge: edge.tuning_params for edge in G}
        # )
        if save_fig:
